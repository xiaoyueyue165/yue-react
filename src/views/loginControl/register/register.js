...
class register_withForm extends React.Component {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      param = {
        sex: this.state.defaultSex,
        identity_front: imageUrl1_url,
        identity_rear: imageUrl2_url,
        business_license: imageUrl3_url,
        ...values,
      };
  ...
  sendVscode() {
    ...
    fetch
      .post(`/api/${API.sendVcode}`, { mobile: this.state.phone })
      .then(res => {
      ...
      <div className="mainBox">
        ...
        <Form onSubmit={this.handleSubmit}>
          <div className="leftName">
            <div className="keyName">公司名称</div>
            <div className="keyName">负责人姓名</div>
            <div className="keyName">职位名称</div>
            <div className="keyName">公司电话</div>
            <div className="keyName">性别</div>
            <div className="keyName">手机号</div>
            <div className="keyName">验证码</div>
            <div className="keyName uploadImg">上传身份证照片</div>
            <div className="keyName uploadImg">上传公司营业执照</div>
            <div className="keyName">设置密码</div>
            <div className="keyName">再次输入</div>
          </div>
          <div className="rightVal">
            ...
            <Form.Item>
              {getFieldDecorator('code', {
                ...
                <div className="code_wrap">
                  <Input className="inputText" placeholder="请输入验证码" />{' '}
                  <div
                    className="sendVscode_register"
                    onClick={() => this.sendVscode()}
                  >
                    发送验证码
                  </div>
                </div>,
            ...
            <div className="uploadImg oneLineBetween">
              <UploadImg
                imgNameKey="imageUrl1_url"
                uploadPrompt="身份证正面图片"
                onSubOnlineImgUrl={this.onSubOnlineImg}
              />
              <UploadImg
                imgNameKey="imageUrl2_url"
                uploadPrompt="身份证背面图片"
                onSubOnlineImgUrl={this.onSubOnlineImg}
              />
            </div>
            <div className="uploadImg oneLineStart">
              <UploadImg
                imgNameKey="imageUrl3_url"
                uploadPrompt="营业执照图片"
                onSubOnlineImgUrl={this.onSubOnlineImg}
              />
            </div>
            <Form.Item>
              {getFieldDecorator('password', {
                ...
                <Input.Password
                  className="inputText"
                  placeholder="请输入密码"
                  onChange={this.handleTwicePwd.bind(this, 'pwd')}
                />,
              )}
            </Form.Item>
            <Form.Item>
              {getFieldDecorator('password2', {
                ...
                <Input.Password
                  className="inputText"
                  placeholder="请确认输入密码"
                  onChange={this.handleTwicePwd.bind(this, 'pwd2')}
                />,
              )}
            </Form.Item>
            ...
            <Agreement
              className="xiayi center"
              text="平台协议"
              type={1}
              checked={AgreementState}
              onPubXieYiState={this.onSubXieyiState}
            />
          </div>
        </Form>
      </div>
