    ...
    */
    loginByPwd = (e) => {
        ...
        if (!y.state.phone || !y.state.vcode) {
            ...
            return;
        } else if (!Utils.checkType(y.state.phone, 'phone')) {
            message.error("请填写正确的手机号码！")
            return;
        }

        fetch.post(`/api/${API.login}`, param).then(res => {
            if (res.data.error_msg) {
                message.error(res.data.error_msg);
                return;
            }
            if (res.data.request_id) {
                let token = res.data.response_data.access_token;
    ...
    */
    loginByVcode = (e) => {
        ...
        if (!y.state.phone2) {
            ...
            return;
        } else if (!Utils.checkType(y.state.phone2, 'phone')) {
            message.error("请填写正确的手机号码！")
            return;
        }
        if (!y.state.vcode2) {
    ...
    sendVscode() {
        ...
        fetch.post(`/api/${API.sendVcode}`, { mobile: this.state.phone2 }).then(res => {
    ...
    render() {
        ...
        const tabContent_pwd = <div className="signInTab_tabContentBox columnAround">
            <Input className="phone" value={phone} onChange={this.onChange.bind(this, 'phone')} placeholder="请输入手机号" />
            <img className="phone_icon" src={phoneIcon} />
            <div className="pwd_wrap">
                <Input.Password className="pwd" onChange={this.onChange.bind(this, 'vcode')} placeholder="请输入密码" />
            </div>
            <img className="phone_icon icon_pwd" src={pwd} />
            <div className="checkBox">
                <Checkbox onChange={this.onChange.bind(this, 'autoLogin')}>自动登录</Checkbox>
            </div>
            <Button className="loginBtn" onClick={this.loginByPwd} type="primary">登录</Button>
            <div className="tishi oneLineBetween">
                <div>没有账号<span className="blue"><Link to="/register">立即注册</Link></span></div>
                <div className="blue"><Link to="/forgetPwd">忘记密码</Link></div>
            </div>
        </div>;
        const tabContent_vcode = <div className="signInTab_tabContentBox columnAround">
            <Input className="phone" value={phone2} onChange={this.onChange.bind(this, 'phone2')} placeholder="请输入手机号" />
            <img className="phone_icon" src={phoneIcon} />
            {error_msg ? <div className="phone_toast oneLineEnd"><img className="icon_tan" src={tan} /> {error_msg}</div> : ''}

            <Input className="vcode" placeholder="请输入验证码" onChange={this.onChange.bind(this, 'vcode2')} />
            <div className="sendVscode" onClick={() => this.sendVscode()}>发送验证码</div>
            <img className="icon_vcode" src={msg} />
            <Button className="loginBtn2" onClick={this.loginByVcode} type="primary">登录</Button>
        </div>;
