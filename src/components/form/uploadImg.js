...
class UploadImg extends Component {
    constructor(props) {
        super(props);
        this.state = {
            Common_uploadImgUrl: process.env.NODE_ENV === "production" ? `${API.uploadImg}` : `/api/${API.uploadImg}`, // 本项目一致上传地址，不修改
            loading: false
        }
    }
    static propTypes = {
    ...
    */
    UploadImghandleChange = (BaseImgName, info) => {
        ...
        if (info.file.status === 'done') {
            ...
            // 获取上传后图片真实的在线地址
            let response_newOnlineImgUrl = info.file.response.response_data;
            let localKey = BaseImgName + '_local';
            let onlineKey = BaseImgName + '_online';
            getBase64(info.file.originFileObj, localImage => this.setState({
    ...
    onPubOnlineImgUrl = (name, onlineObj) => {
        let onlineImgMsg = { 'name': name, ...onlineObj }
    ...
    render() {
        ...
        return (
            ...
            >
                {this.state[`${imgNameKey}_local`] ? <img src={this.state[`${imgNameKey}_local`]} alt="avatar" /> : <UploadButton showName={uploadPrompt} loading={loading} />}
            </Upload>
        )
    }
}
