...
class AccountType extends Component {
    ...
    render() {
        ...
        if (this.props.defaultId) {
            children = <Select placeholder={placeholderShow} value={this.props.defaultId} className={this.props.className} style={{ width: 120 }} onChange={this.handleChangeAccountType}>
            ...
            // 无初始值
            children = <Select placeholder={placeholderShow} className={this.props.className} style={{ width: 120 }} onChange={this.handleChangeAccountType}>
                {accountTypes.map(item => {
                    return <Option key={item.id} value={item.id}>{item.title}</Option>
                })}
